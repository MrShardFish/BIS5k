...
class PAA_f(nn.Module):
    def __init__(self, in_channel, out_channel):
        super(PAA_f, self).__init__()
        self.relu = nn.ReLU(True)
        ...
        self.conv_cat = conv(4 * out_channel, out_channel, 3, stride=2)
        self.conv_res = conv(in_channel, out_channel, 1, stride=2)
...
class TIR(nn.Module):
    def __init__(self, in_channel, out_channel):
        ...
        self.conv_cat = conv(3 * out_channel, out_channel, 3, stride=1)
        self.conv_res = conv(in_channel, out_channel, 1, stride=1)
